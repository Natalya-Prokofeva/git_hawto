...
```
__*Обязательно нужно помнить, что для этого сначало нужно вызвать нужную ветку (git checkout branch_name) и уже в ней вводить команду (git merge branch_name) иначе может возникнуть конфлиикт.*__
