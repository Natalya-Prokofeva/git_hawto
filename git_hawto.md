...
```sh
git pull -команда для того, чтобы синхронизировать удаленный и локальный репозитории. Также эта команда является составной и она не только подгрузит все изменения, но и попытается смержить наши ветки т.е. слить все ветки.
```
